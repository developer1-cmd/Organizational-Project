...
function EmployeeAnnouncementsPage() {
  ...
  // Drawer drag state
  const [drawerPosition, setDrawerPosition] = useState({ x: 50, y: 200 });
    ...
    <div style={styles.layout}>
      <div style={styles.mainContent}>
        ...
        <div
          ...
          onMouseMove={(e) => {
            if (isDragging) {
              setDrawerPosition({ x: e.clientX - dragOffset.x, y: e.clientY - dragOffset.y });
            }
            if (isDraggingButton) {
              setButtonPosition({
                x: e.clientX - buttonPosition.x + buttonPosition.x,
                y: e.clientY - buttonPosition.y + buttonPosition.y,
              });
            }
          }}
          onMouseUp={() => {
        ...
        >
          ...
          <div
            ...
            onTouchMove={(e) => {
              if (isDragging) {
                ...
                setDrawerPosition({ x: touch.clientX - dragOffset.x, y: touch.clientY - dragOffset.y });
              }
              if (isDraggingButton && e.touches) {
                const touch = e.touches[0];
                setButtonPosition({
                  x: touch.clientX - buttonPosition.x + buttonPosition.x,
                  y: touch.clientY - buttonPosition.y + buttonPosition.y,
                });
              }
            }}
            onTouchEnd={() => {
              ...
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px', gap: '12px', flexWrap: 'wrap' }}>
                  ...
                  <input
                    ...
                    style={{ padding: '8px 12px', borderRadius: '6px', border: '1px solid #ccc', flexGrow: 1 }}
                  />
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  {announcements
                    ...
                    .sort((a, b) => {
                      const isPinnedA = pinned.includes(a.title);
                      const isPinnedB = pinned.includes(b.title);
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            <button
                              ...
                              style={{
                                ...
                                cursor: 'pointer',
                                fontSize: '16px',
                                color: pinned.includes(item.title) ? '#e1b12c' : '#ccc',
                              }}
                              title={pinned.includes(item.title) ? 'Unpin' : 'Pin'}
                            >
                              📌
                            </button>
                          </div>
          ...
          {viewEmployees && (
            <div
              ...
              onMouseDown={(e) => {
                const offsetX = e.clientX - buttonPosition.x;
                const offsetY = e.clientY - buttonPosition.y;
                const onMove = (moveEvent) => {
                  setButtonPosition({ x: moveEvent.clientX - offsetX, y: moveEvent.clientY - offsetY });
                };
                const onUp = () => {
              ...
              onClick={() => {
                if (!isDraggingButton) setShowEmployeeDrawer(true);
              }}
            >
              👥
            </div>
          ...
          {viewEmployees && showEmployeeDrawer && (
            ...
            >
              ...
              <div
                ...
                onMouseDown={(e) => {
                  setIsDragging(true);
                  setDragOffset({ x: e.clientX - drawerPosition.x, y: e.clientY - drawerPosition.y });
                }}
              >
